...
(function () {
  ...
  function renderHeader() {
    ...
    if (!mount) return;
    const visibleNavItems = NAV_ITEMS.filter(([label, href, page]) => {
    ...
    const links = visibleNavItems.map(([label, href, page]) => `<a class="nav-link ${currentPage === page ? "is-active" : ""}" href="${hrefFor(href, page)}">${label}</a>`).join("");
    ...
    mount.innerHTML = `
      <header class="site-header" data-header>
        <div class="container header-inner">
          ...
          <nav class="main-nav" aria-label="Menu principal" data-main-nav>${searchBox}${links}</nav>
  ...
  function siteSearchIndex() {
    const items = NAV_ITEMS.map(([label, href, page]) => ({
      title: label,
      text: {
        home:"Página inicial",
